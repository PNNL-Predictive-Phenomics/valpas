...
def analyze_protein_relationships(
    ...
    """
    Analyze protein relationships based on similarity matrix

    Args:
        similarity_matrix: Protein similarity matrix from autoencoder
        original_data: Original proteomics data
        top_n: Number of top similar pairs to return
        threshold: Similarity threshold for defining relationships

    Returns:
        Dictionary with relationship analysis
    """

    protein_names = similarity_matrix.index.tolist()

    # Find highly similar protein pairs
    similar_pairs = []
        ...
        for j in range(i+1, len(protein_names)):
            ...
            if similarity > threshold:
                similar_pairs.append({
                    'protein1': protein_names[i],
                    'protein2': protein_names[j],
    ...
    top_similar_pairs = similar_pairs[:top_n]

    # Analyze expression patterns for top pairs
    pair_analysis = []
    for pair in top_similar_pairs:
        idx1, idx2 = pair['protein1_idx'], pair['protein2_idx']

        expr1 = original_data.iloc[idx1].values
        expr2 = original_data.iloc[idx2].values

        # Calculate expression correlation
        expr_correlation = np.corrcoef(expr1, expr2)[0, 1]

        # Calculate expression statistics
        mean_diff = np.mean(np.abs(expr1 - expr2))
        max_diff = np.max(np.abs(expr1 - expr2))

        pair_analysis.append({
            'pair': f"{pair['protein1']} - {pair['protein2']}",
            'autoencoder_similarity': pair['similarity'],
            'expression_correlation': expr_correlation,
            'mean_expression_diff': mean_diff,
            'max_expression_diff': max_diff,
            'protein1_mean': np.mean(expr1),
            'protein2_mean': np.mean(expr2),
            'protein1_std': np.std(expr1),
            'protein2_std': np.std(expr2)
        })

    # Overall statistics
    all_similarities = similarity_matrix.values[np.triu_indices_from(similarity_matrix.values, k=1)]
...
def visualize_protein_relationships(
    similarity_matrix: pd.DataFrame,
    analysis_results: Dict,
    figsize: Tuple[int, int] = (15, 12)
) -> plt.Figure:
    """
    Create comprehensive visualizations of protein relationships
    """
    ...
    # For large matrices, subsample for visualization
    if len(similarity_matrix) > 100:
        indices = np.random.choice(len(similarity_matrix), 100, replace=False)
        plot_matrix = similarity_matrix.iloc[indices, indices]
    ...
    ax2.hist(all_similarities, bins=50, alpha=0.7, edgecolor='black')
    ax2.axvline(np.mean(all_similarities), color='red', linestyle='--',
                label=f'Mean: {np.mean(all_similarities):.3f}')
    ax2.set_xlabel('Similarity')
    ax2.set_ylabel('Frequency')
    ax2.set_title('Distribution of Protein Similarities')
    ax2.legend()
    ax2.grid(True, alpha=0.3)

    # 3. Top similar pairs
    ax3 = plt.subplot(2, 3, 3)
    top_pairs = analysis_results['pair_analysis'][:15]

    if top_pairs:
        pairs = [p['pair'][:20] + '...' if len(p['pair']) > 20 else p['pair'] for p in top_pairs]
        similarities = [p['autoencoder_similarity'] for p in top_pairs]

        bars = ax3.barh(range(len(pairs)), similarities, color='lightcoral')
        ax3.set_yticks(range(len(pairs)))
        ax3.set_yticklabels(pairs, fontsize=8)
        ax3.set_xlabel('Autoencoder Similarity')
        ax3.set_title('Top Similar Protein Pairs')
        ax3.grid(True, alpha=0.3, axis='x')

        # Add value labels
        for i, (bar, sim) in enumerate(zip(bars, similarities)):
            ax3.text(sim + 0.01, i, f'{sim:.3f}', va='center', fontsize=8)

    # 4. Similarity vs Expression Correlation
    ax4 = plt.subplot(2, 3, 5)
    if analysis_results['pair_analysis']:
        autoenc_sims = [p['autoencoder_similarity'] for p in analysis_results['pair_analysis']]
        expr_corrs = [p['expression_correlation'] for p in analysis_results['pair_analysis']]

        ax4.scatter(expr_corrs, autoenc_sims, alpha=0.6)
        ax4.set_xlabel('Expression Correlation')
        ax4.set_ylabel('Autoencoder Similarity')
        ax4.set_title('Autoencoder Similarity vs Expression Correlation')
        ax4.grid(True, alpha=0.3)

        # Add correlation coefficient
        overall_corr = np.corrcoef(expr_corrs, autoenc_sims)[0, 1]
        ax4.text(0.05, 0.95, f'r = {overall_corr:.3f}', transform=ax4.transAxes,
    ...
    Total Proteins: {stats['n_proteins']}
    Similar Pairs (>{stats['threshold_used']}): {stats['n_similar_pairs']}

    Similarity Statistics:
    Mean: {stats['mean_similarity']:.4f}
    Std: {stats['std_similarity']:.4f}
    Min: {stats['min_similarity']:.4f}
    Max: {stats['max_similarity']:.4f}

    Top Similar Pair:
    {analysis_results['top_similar_pairs'][0]['protein1']} -
    {analysis_results['top_similar_pairs'][0]['protein2']}
    Similarity: {analysis_results['top_similar_pairs'][0]['similarity']:.4f}
    """
