...
def _read_excel_file(filepath: str, sheet_name: Union[str, int], header: Union[int, List[int]],
...
def _read_text_file(filepath: str, delimiter: str, file_ext: str, header: Union[int, List[int]],
                   skiprows: int, na_values: Union[str, List[str]], encoding: str,
                   read_kwargs: Dict, verbose: bool) -> pd.DataFrame:
...
def _detect_delimiter(filepath: str, file_ext: str, verbose: bool) -> str:
...
def _validate_and_process_dataframe(
    df: pd.DataFrame,
    primary_id_column: str,
    primary_annotation_column: str,
...
def _validate_primary_ids(df: pd.DataFrame, primary_id_column: str,
                         remove_duplicates: str, verbose: bool) -> Dict:
    """Validate primary ID column"""

    id_validation = {
        'total_ids': len(df),
        'unique_ids': df[primary_id_column].nunique(),
        'missing_ids': df[primary_id_column].isna().sum(),
        'empty_ids': 0,
        'n_duplicates': 0,
        'duplicate_ids': []
    }
    ...
    if df[primary_id_column].dtype == 'object':
        empty_mask = (df[primary_id_column] == '') | (df[primary_id_column] == ' ')
        id_validation['empty_ids'] = empty_mask.sum()

    # Check for duplicates
    duplicated_mask = df.duplicated(subset=[primary_id_column], keep=False)
    id_validation['n_duplicates'] = duplicated_mask.sum()

    if id_validation['n_duplicates'] > 0:
        duplicate_ids = df.loc[duplicated_mask, primary_id_column].unique().tolist()
        id_validation['duplicate_ids'] = duplicate_ids[:10]  # First 10 duplicates

    return id_validation

def _perform_data_quality_checks(df: pd.DataFrame, primary_id_column: str,
                                primary_annotation_column: str) -> Dict:
    """Perform additional data quality checks"""

    quality_checks = {}

    # Check annotation diversity
    unique_annotations = df[primary_annotation_column].nunique()
    total_non_null = df[primary_annotation_column].notna().sum()
    ...
    if df[primary_annotation_column].dtype == 'object':
        annotations = df[primary_annotation_column].dropna().astype(str)
    ...
    if df[primary_id_column].dtype == 'object':
        ids = df[primary_id_column].dropna().astype(str)
...
def _print_validation_summary(validation_report: Dict):
...
if __name__ == "__main__":
    ...
    # Sample 1: CSV file
    sample_data = {
        'protein_id': ['P12345', 'P67890', 'Q12345', 'Q67890', 'R12345'],
        'gene_name': ['GENE1', 'GENE2', 'GENE3', 'GENE4', 'GENE5'],
        'function': ['Enzyme', 'Receptor', 'Transcription factor', 'Enzyme', 'Transporter'],
        'pathway': ['Glycolysis', 'Signal transduction', 'Gene regulation', 'Metabolism', 'Transport'],
        'organism': ['Human', 'Human', 'Mouse', 'Human', 'Mouse']
    }

    sample_df = pd.DataFrame(sample_data)
    sample_df.to_csv('test_annotations.csv', index=False)

    # Sample 2: TSV file with some issues
    problematic_data = {
        'Protein_ID': ['P12345', 'P67890', 'P12345', '', 'Q67890'],  # Duplicate and empty
        'Gene_Name': ['GENE1', 'GENE2', 'GENE1', 'GENE4', None],    # Missing annotation
        'Description': ['Good protein', 'Another protein', 'Duplicate entry', 'Empty ID protein', 'Missing name'],
        'Extra_Info': ['Info1', 'Info2', 'Info3', 'Info4', 'Info5']
    }

    problematic_df = pd.DataFrame(problematic_data)
    problematic_df.to_csv('test_annotations_issues.tsv', sep='\t', index=False)

    # Sample 3: Excel file
    with pd.ExcelWriter('test_annotations.xlsx') as writer:
        sample_df.to_excel(writer, sheet_name='Annotations', index=False)
        # Add a second sheet
        extra_data = pd.DataFrame({'col1': [1, 2, 3], 'col2': ['a', 'b', 'c']})
    ...
    print("\n" + "="*60)
    print("TEST 1: Basic CSV reading")
    print("="*60)

    try:
        df1, report1 = read_annotation_file(
            filepath='test_annotations.csv',
            primary_id_column='protein_id',
            primary_annotation_column='gene_name',
            required_columns=['function'],
            optional_columns=['pathway', 'organism', 'nonexistent_col'],
            verbose=True
        )

        print(f"\nLoaded dataframe shape: {df1.shape}")
    ...
    # Test 2: TSV with issues
    print("\n" + "="*60)
    print("TEST 2: TSV with data issues")
    print("="*60)

    try:
        df2, report2 = read_annotation_file(
            filepath='test_annotations_issues.tsv',
            primary_id_column='Protein_ID',
            primary_annotation_column='Gene_Name',
        ...
        print(f"\nLoaded dataframe shape: {df2.shape}")
    ...
    # Test 3: Excel file
    print("\n" + "="*60)
    print("TEST 3: Excel file reading")
    print("="*60)

    try:
        df3, report3 = read_annotation_file(
            filepath='test_annotations.xlsx',
            primary_id_column='protein_id',
            primary_annotation_column='gene_name',
        ...
        print(f"\nLoaded dataframe shape: {df3.shape}")
        print(f"Columns: {list(df3.columns)}")

    except Exception as e:
        print(f"Error: {e}")

    # Test 4: Case insensitive column matching
    print("\n" + "="*60)
    print("TEST 4: Case insensitive column matching")
    print("="*60)

    try:
        df4, report4 = read_annotation_file(
            filepath='test_annotations.csv',
            primary_id_column='PROTEIN_ID',  # Different case
            primary_annotation_column='GENE_NAME',  # Different case
            case_sensitive=False,
            verbose=True
        )

        print(f"\nLoaded dataframe shape: {df4.shape}")
    ...
    try:
        df5, report5 = read_annotation_file(
            filepath='test_annotations.csv',
            primary_id_column='nonexistent_column',
            primary_annotation_column='gene_name',
            verbose=True
        )
    ...
    # Clean up test files
    import os
    test_files = ['test_annotations.csv', 'test_annotations_issues.tsv', 'test_annotations.xlsx']
